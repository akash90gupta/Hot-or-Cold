...
//Gets whats in the text box and calls the function submit
	...
	info();
//Sets the number in the text box to variable guess and calls validate to see if the number is between 1 and 100 inclusive
	function submit(){
		var guess = $('#guess').val();
		validate(guess);
	}
...
//gives the initial feedback to the user whether thier guess is to large or small and increments the attmepts by 1
	function firstGuess(guess, answer){
		if(guess < answer){
			$('#info').html('Your guess was too SMALL').css({color:'#000'}).effect('highlight', {color: '#FFFFE0'}, 4000);
		}
		else if(guess > answer){
			$('#info').html('Your guess was too LARGE').css({color:'#000'}).effect('highlight', {color: '#FFFFE0'}, 4000);
		}
		attempts++;
...
//tell the user whether they are getting closer or further from answer. Also tells them if they are to high or low
	function hotOrCold(prev_distance, distance, guess, answer){
		if(prev_distance > distance){
			if(guess > answer)
				$('#info').html('You are getting closer! But you are to high!').css({color:'#000'}).effect('highlight', {color: '#FF0000'}, 4000);
			else
				$('#info').html('You are getting closer! But you are to low!').css({color:'#000'}).effect('highlight', {color: '#FF0000'}, 4000);
		}
		if(prev_distance < distance){
			if(guess > answer)
				$('#info').html('You are getting further from the answer! And you are to high!').css({color:'#000'}).effect('highlight', {color: '#0000FF'}, 4000);
			else
				$('#info').html('You are getting further from the answer! And you are to low!').css({color:'#000'}).effect('highlight', {color: '#0000FF'}, 4000);
		}
		attempts++;
	}
